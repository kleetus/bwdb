...
WebWorker.prototype.start = function(callback) {
  ...
  async.series([
    function(next) {
      log.info('Connecting web worker socket to web socket');
...
//everything that needs to write to lmdb needs to get queued on the writer, which involves sending a message
//to the writer that contains a task and a priority.
//There is only one writer, so in order to have any semble of serving a priority queue, jobs in the writer's
//must be relatvely small jobs requiring small amounts of time to run.
//Since some of those jobs involving interacting with a remote server (bitcoind's rpc server), jobs in writer's
//queue cannot guarantee a "short" running time.
...
WebWorker.prototype._queueBatchWriterTask = function(method, params, priority) {
  var self = this;
  assert(Array.isArray(params), '"params" is expected to be an array');
...
WebWorker.prototype.getWalletTransactions = function(walletId, options, callback) {
  ...
  this._getLatestTxids(txn, walletId, options, function(err, result) {
    ...
    async.map(result.txids, function(txidInfo, next) {
      var txid = txidInfo[2];
      var key = models.WalletTransaction.getKey(walletId, txid);
      var buffer = txn.getBinary(self.db.txs, key);
      if (!buffer) {
        next(null, {});
        //we may be still gathering the tx data, so send an empty object for now
      } else {
...
WebWorker.prototype._endpointJobStatus = function() {
  var self = this;
  return function(req, res) {
    var jobId = req.params.jobId;
    self._queueWriterTask('getJobStatus', [jobId], 1, function(err, status) {
      if (err) {
        return res.status(503).jsonp(err);
      }
      if (!status) {
...
/*
   _endpointPostAddresses:
   inputs: file upload handled by multer. Will only allow a single file with the field or key, "addresses"
   file data is stored in memory. Access it via req.file, the file's contents will be at req.file.buffer
   outputs: will quickly return a jobId for later querying of the job status.
...
WebWorker.prototype._endpointPostAddresses = function() {
  ...
  return function(req, res) {
    ...
    var walletId = req.walletId;
    var idHex = walletId.toString('hex');
    var jobId = self._queueBatchWriterTask('importWalletAddresses', [idHex, addresses], 2);
    res.status(201).jsonp({
      jobId: jobId
    });
  };
